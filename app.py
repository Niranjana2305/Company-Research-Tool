...
try:
    with engine.connect() as conn:
        if engine.dialect.name == "sqlite":
            company_cols = {
                row[1] for row in conn.execute(sa_text("PRAGMA table_info(company);")).fetchall()
            }
            employee_cols = {
                row[1] for row in conn.execute(sa_text("PRAGMA table_info(employee);")).fetchall()
            }

            if "email" not in company_cols:
                conn.execute(sa_text("ALTER TABLE company ADD COLUMN email VARCHAR(255);"))
            if "name_normalized" not in company_cols:
                conn.execute(
                    sa_text("ALTER TABLE company ADD COLUMN name_normalized VARCHAR(255);")
                )
            if "email" not in employee_cols:
                conn.execute(sa_text("ALTER TABLE employee ADD COLUMN email VARCHAR(255);"))
        else:
            conn.execute(
                sa_text("ALTER TABLE company ADD COLUMN IF NOT EXISTS email VARCHAR(255);")
            )
            conn.execute(
                sa_text(
                    "ALTER TABLE company ADD COLUMN IF NOT EXISTS name_normalized VARCHAR(255);"
                )
            )
            conn.execute(
                sa_text("ALTER TABLE employee ADD COLUMN IF NOT EXISTS email VARCHAR(255);")
            )
        ...
        try:
            conn.execute(
                sa_text(
                    "UPDATE company SET name_normalized = lower(name) "
...
def parse_employee_list_from_text(text: str) -> List[Dict[str, Any]]:
    ...
    for line in text.splitlines():
        ...
        if not emp["email"]:
            m = re.search(
                r"([a-zA-Z0-9.\-_+]+@[a-zA-Z0-9\-_]+\.[a-zA-Z0-9.\-_]+)", line
            )
            if m:
                emp["email"] = m.group(1)
...
def upsert_company_and_employees(data: Dict[str, Any], raw_user_input: str = "") -> Company:
    ...
    normalized_user_input = (
        normalize_company_key(raw_user_input) if raw_user_input else None
    )
    ...
    with Session(engine) as session:
        ...
        if normalized_user_input and has_name_normalized_field():
            existing = session.exec(
                select(Company).where(
                    Company.name_normalized == normalized_user_input
                )
            ).first()
        ...
        if not existing:
            existing = session.exec(
                select(Company).where(
                    func.lower(Company.name) == (company_name or "").lower()
                )
            ).first()
        ...
        if existing:
            ...
            if has_name_normalized_field():
                ...
                elif not getattr(existing, "name_normalized", None):
                    existing.name_normalized = normalize_company_key(existing.name)
            ...
            name_normalized_value = normalized_user_input or normalize_company_key(
                company_name
            )
            ...
            company_obj = Company(
                name=company_name,
                **(
                    {"name_normalized": name_normalized_value}
                    if has_name_normalized_field()
                    else {}
                ),
        ...
        for e in employees:
            ...
            if existing_emp:
                ...
                session.add(Employee(**new_emp_kwargs))
